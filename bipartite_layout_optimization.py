"""
二部グラフに対する多目的最適化に基づくレイアウト生成(一般的な可読性指標版)

概要:
    MovieLens ml-1m データセット(ユーザー・映画の二部グラフ)からサブグラフを
    抽出し、NSGA-II を用いて、グラフ描画分野で広く使われる標準的な可読性指標
    (エッジ交差数・エッジ長の均一性・ノードの重なり回避)を目的関数として
    同時最適化するサンプル実装です。

    本リポジトリは、卒業研究で検討している二部グラフ特有のクラスタベース評価
    指標(エッジクラスタリングに基づく方向整列など)は含んでいません。ここでは
    MovieLensデータからの抽出処理と、NSGA-IIによる多目的最適化の一般的な
    実装例のみを公開しています。
