"""
二部グラフに対する多目的最適化に基づくレイアウト生成(一般的な可読性指標のみを掲載)

概要:
    MovieLens データセット(ユーザー・映画の二部グラフ)からサブグラフを抽出し、NSGA-II を用いて、
    グラフ描画分野で広く使われる標準的な可読性指標(エッジ交差数・エッジ長の均一性・ノードの重なり回避)を目的関数として同時最適化するサンプル実装です。
    本リポジトリは、二部グラフ特有の評価指標は含んでいません。
    ここではMovieLensデータからの抽出処理と、NSGA-IIによる多目的最適化の一般的な実装例のみを公開しています。
