...
グラフ描画の可読性(aesthetics)を評価する複数の指標は互いにトレードオフの関係に
あることが知られています。本リポジトリは、NSGA-II を用いた多目的最適化によって、
二部グラフに対して複数のレイアウト候補(パレート解)を同時に生成する手法のサンプル実装です。
...
- グラフ描画分野で広く使われる標準的な可読性指標
  ...
  - エッジ長の均一性
- 得られたパレート解集合に対する相関分析(ピアソン・スピアマン)、
  解の多様性(距離分布)分析、主成分分析(PCA)

**注記**: 卒業研究で検討している、二部グラフのエッジ構造に着目した独自のクラスタリング手法や評価指標(エッジクラスタリングに基づく方向整列など)は、研究の新規性に関わる部分のため本リポジトリには含めていません。ここで公開しているのは、データ抽出処理と、NSGA-IIを用いた多目的最適化の一般的な実装例です。
