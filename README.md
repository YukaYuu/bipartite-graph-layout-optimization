...
**注記**: 卒業研究で検討している、二部グラフのエッジ構造に着目した独自の
クラスタリング手法や評価指標(エッジクラスタリングに基づく方向整列など)は、
研究の新規性に関わる部分のため本リポジトリには含めていません。ここで公開して
いるのは、データ抽出処理と、NSGA-IIを用いた多目的最適化の一般的な実装例です。
